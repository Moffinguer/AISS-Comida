...
public class PlaylistResource {
	...
	public Collection<Playlist> getAll(@QueryParam("order") String order,
			@QueryParam("isEmpty") String empty, @QueryParam("name") String  name)
	{
		Collection<Playlist> playlists = repository.getAllPlaylists();
		if(empty != null) {
			if(empty.equals("true")) {
				playlists = playlists.stream().filter(pl -> pl.getSongs().size() == 0).collect(Collectors.toList());
			}else if(empty.equals("false")) {
				playlists = playlists.stream().filter(pl -> pl.getSongs().size() != 0).collect(Collectors.toList());
			}
		}
		if(name  != null) {
		...
		if(order   != null) {
			if(order.equals("name")) {
				...
				playlists = playlists.stream().sorted(Comparator.comparing(Playlist::getName).reversed()).collect(Collectors.toList());
			}
		}
		return playlists;
	}
