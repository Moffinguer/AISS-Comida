...
public class PlatoResource {
	...
	{				
		...
		try {
			...
			throw new BadRequestException("La cantidad debe ser un n√∫mero");
	    }
	...
	public Response updateDish(Plato nuevoPlato) {
		if(nuevoPlato == null) {
			throw new BadRequestException("No se ha enviado ninguna modificaci√≥n");
		}
...
//		checkFieldsNotFilled(nuevoPlato);
		...
		if(nuevoPlato.getAlimentos() != null) {
			...
			try {
				...
				throw new BadRequestException("No ha introducido ning√∫n ingrediente");
			}
	...
	public Response addPlato(@Context UriInfo uriInfo, Plato plato) {	
		...
		if(plato.getAlimentos() == null || plato.getAlimentos().isEmpty()) throw new BadRequestException("La lista de alimentos no puede ser nula o estar vacÌa");
		if(plato.getCAOrigen() == null || plato.getCAOrigen().equals("")) throw new BadRequestException("La comunidad autÛnoma de origen del plato no puede ser nula");
